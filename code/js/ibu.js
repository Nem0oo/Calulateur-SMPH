...
ibu._construct = function() {
  ...
  this.kettleDiameter.defaultValue = 34.1;
  ...
  this.kettleOpening.defaultValue = 34.1;
...
function hopAdditionsSet(param) {
  ...
  if (constructInputTable &&
    ...
    table += "</tr> ";
    table += "<tr> ";
    table += "<td> <a href='hop_freshness.html'>Facteur de fraicheur</a>: </td> "
  }
  for (idx = 1; idx <= numAdd; idx++) {
    arrayIdx = Number(idx-1);
  ...
  if (constructInputTable) {
    table += "</tr> "
    table += "<tr> "
    table += "<td> Poid (<span id='weightUnits'>"+weightUnits+"</span>):</td> "
  }
  for (idx = 1; idx <= numAdd; idx++) {
    arrayIdx = Number(idx-1);
